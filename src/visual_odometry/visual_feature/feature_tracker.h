...
public:
    ...
    //获取深度,重点是激光雷达系为前左上，相机为右下前.雷达前左上-->相机前左上-->相机右下前
    // 1.【W系雷达前左上】转【相机前左上】得雷达球体点云

    // 2.【相机右下前】转【相机前左上】得相机球体点云

    // 3.在【雷达球体点云】中寻找【相机球体点云】深度信息

    // 4.将【相机前左上】的【相机球体点云】转到【相机右下前】画在图像中。
    sensor_msgs::ChannelFloat32 get_depth(const ros::Time& stamp_cur, 
                                          const cv::Mat& imageCur, //
    ...
    {
        // 0.1 initialize depth for return   1.初始化需要返回的每个特征点的深度信息
        sensor_msgs::ChannelFloat32 depth_of_point;
        depth_of_point.name = "depth";
        depth_of_point.values.resize(features_2d.size(), -1);//和图像特征一一匹配

        // 0.2  check if depthCloud available  2.检查点云图是否可用
        if (depthCloud->size() == 0)
            return depth_of_point;

        // 0.3 look up transform at current image time 3.在当前图像时间查找tf变换，
        try{  // 1、当前雷达相对W系变换
        ...
        //把2D图像特征归一化映射到单位球，得features_3d_sphere
        // 0.4 transform cloud from global frame to camera frame 4.将点云图从世界坐标系转换到当前相机坐标系
        ...
        //这里拿到的是一个雷达点云的时间截断处理    5.将未失真的归一化 (z) 2d 特征投影到单位球体上
        ...
        for (int i = 0; i < (int)features_2d.size(); ++i)
        {
            ...
            PointType p;
            p.x =  feature_cur(2);//划重点！！！隐含了一个右下前到前左上的变换
            p.y = -feature_cur(0);
            p.z = -feature_cur(1);
            p.intensity = -1; // intensity will be used to save depth// intensity 会被用来保存深度信息
            features_3d_sphere->push_back(p);
        }

        // 0.6. project depth cloud on a range image, filter points satcked in the same region投影深度点云图在距离图像上并过滤位于同一区域的点
        //雷达点云网格化；
        //过滤不在相机视野中的点，然后，在图像的范围中找到行号，在图像的范围中找到列号。但是找到的序列号（包括行和列）可能超过限制范围，需要过滤。最后，只保留较近的点
        float bin_res = 180.0 / (float)num_bins; // currently only cover the space in front of lidar (-90 ~ 90)// 只能覆盖lidar前面的 (-90 ~ 90) 区域
        ...
        for (int i = 0; i < (int)depth_cloud_local->size(); ++i)
        {
            ...
            // filter points not in camera view// 过滤不在相机视野中的点
            if (p.x < 0 || abs(p.y / p.x) > 10 || abs(p.z / p.x) > 10)
                continue;
            // find row id in range image// 在图像的范围中找到行号，俯仰角离散
            float row_angle = atan2(p.z, sqrt(p.x * p.x + p.y * p.y)) * 180.0 / M_PI + 90.0; // degrees, bottom -> up, 0 -> 360，从底部到顶部
            int row_id = round(row_angle / bin_res);
            // find column id in range image// 在图像的范围中找到列号，水平角离散
            float col_angle = atan2(p.x, p.y) * 180.0 / M_PI; // degrees, left -> right, 0 -> 360从左到右
            int col_id = round(col_angle / bin_res);
            // id may be out of boundary// id 可能超出约束条件
            if (row_id < 0 || row_id >= num_bins || col_id < 0 || col_id >= num_bins)
                continue;
            // only keep points that's closer// 只保留较近的点
            float dist = pointDistance(p);
            if (dist < rangeImage.at<float>(row_id, col_id))
            {
                rangeImage.at<float>(row_id, col_id) = dist;//把点云投射到一个range image, 只包含雷达前180°水平角和俯仰角的0.5度的360*360的网格，=====放入最近点云的深度
                pointsArray[row_id][col_id] = p;//同样是360*360 的网格，和前面的一一对应，存储的是最近点的坐标值，用俯仰角和水平角保存
            }
        }

        // 0.7. extract downsampled depth cloud from range image从距离图像中提取下采样深度点云图
        //投影到单位球面，
        ...
        *depth_cloud_local = *depth_cloud_local_filter2;
        ...
        //0.8. project depth cloud onto a unit sphere将深度点云图投影到单位球体上
        pcl::PointCloud<PointType>::Ptr depth_cloud_unit_sphere(new pcl::PointCloud<PointType>());
        for (int i = 0; i < (int)depth_cloud_local->size(); ++i)
        {
            PointType p = depth_cloud_local->points[i];
            float range = pointDistance(p);//range=sqrt(x^2+y^2+z^2)
            p.x /= range;//x=x/range
            p.y /= range;
            p.z /= range;
            p.intensity = range;//p点强度
            depth_cloud_unit_sphere->push_back(p);
        }
        if (depth_cloud_unit_sphere->size() < 10)
            return depth_of_point;

        //0.9. create a kd-tree using the spherical depth cloud球形深度点云创建 kd 
        pcl::KdTreeFLANN<PointType>::Ptr kdtree(new pcl::KdTreeFLANN<PointType>());
        kdtree->setInputCloud(depth_cloud_unit_sphere);
        // k-d树是每个叶子节点都为k维点的二叉树。所有非叶子节点可以视作用一个超平面把空间分割成两个半空间。
        // 节点左边的子树代表在超平面左边的点，节点右边的子树代表在超平面右边的点。
        // 选择超平面的方法如下：每个节点都与k维中垂直于超平面的那一维有关。
        // 因此，如果选择按照x轴划分，所有x值小于指定值的节点都会出现在左子树，所有x值大于指定值的节点都会出现在右子树
        // 。这样，超平面可以用该x值来确定，其法线为x轴的单位向量。
        
        // 0.10. find the feature depth using kd-tree使用 kd-tree 找到特征深度，并返回结果
        ...
        for (int i = 0; i < (int)features_3d_sphere->size(); ++i)
        {
            ...
            if (pointSearchInd.size() == 3 && pointSearchSqDis[2] < dist_sq_threshold)
            {
                ...
                if (max_depth - min_depth > 2 || s <= 0.5)//太大和太小都不合理
                {
                    continue;
                ...
                // convert feature into cartesian space if depth is available又归一化
