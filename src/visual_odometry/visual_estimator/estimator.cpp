                             ...
                             const std_msgs::Header &header)
{
    ...
    if (f_manager.addFeatureCheckParallax(frame_count, image, td))// 添加特征点, 检测视差 (根据视差决定marge最老帧还是次新帧)
...
bool Estimator::initialStructure()
{
    // Lidar initialization
    {
        ...
        for (int i = 0; i <= WINDOW_SIZE; i++)
        {
                ...
                all_image_frame[Headers[i].stamp.toSec()].reset_id != all_image_frame[Headers[0].stamp.toSec()].reset_id)
            {
                ...
                // 激光雷达里程计不可用 (id=-1) 或激光雷达里程计由于姿态校正而重新定位
    ...
    vector<SFMFeature> sfm_f;//存储滑窗中所有特征点的归一化坐标
...
void Estimator::optimization()
{
    ...
    loss_function = new ceres::CauchyLoss(1.0);//鲁棒和函数，不会拉得太远
    for (int i = 0; i < WINDOW_SIZE + 1; i++)
    {
        // 位姿中包含四元数，四元数是一种过参数化的表示，若按照四元数本身的维度，优化方向会有4个，而实际优化的方向(维度)只有3个;
        // 为了移除多余的空的优化方向，因此需要特殊定义优化过程中其运算的过程；
        ceres::LocalParameterization *local_parameterization = new PoseLocalParameterization();
        problem.AddParameterBlock(para_Pose[i], SIZE_POSE, local_parameterization);//每一次构建残差块，都系要告诉系统怎么进行运算
        problem.AddParameterBlock(para_SpeedBias[i], SIZE_SPEEDBIAS);//默认方式
    ...
    // 边缘化流程：
    // 1.创建marginalization_info，从头到位都是marginalization_info这个变量来进行统筹安排进行边缘化；
    //   然后通过marginalization_info->addResidualBlockInfo()来添加约束，有4个方面的来源：（1）旧的（2）imu预积分项（3）特征点 (4) odom
    // 2.首先通过last_marginalization_info 构建出marginalization_factor，这个factor就是先验的残差项；
    // 3.然后将滑窗内第0帧和第1帧间的 IMU 预积分值(pre_integrations[1])构建预积分因子，然后构建imu残差项；
    // 4.接着挑选出第一次观测帧为第 0 帧的路标点，和其他共视帧构建重投影因子，然后构建视觉残差项；
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
...

        {
            if (pre_integrations[1]->sum_dt < 10.0)
            {
                ...
                // 残差块信息包含了要边缘化的帧与后一帧的残差信息，相关的参数块，以及要丢弃的变量块；
