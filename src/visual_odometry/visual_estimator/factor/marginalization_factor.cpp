...
void MarginalizationInfo::addResidualBlockInfo(ResidualBlockInfo *residual_block_info)
{
    ...
    for (int i = 0; i < static_cast<int>(residual_block_info->parameter_blocks.size()); i++)
    {
        ...
        parameter_block_size[reinterpret_cast<long>(addr)] = size; // 将各个参数块的地址和大小构造成map容器，键为地址，值为大小；
    }

    // 对于每一个要边缘化的参数块；drop_set 这个向量中存的是要边缘化的参数块的块地址，比如第0块和第1块；
    for (int i = 0; i < static_cast<int>(residual_block_info->drop_set.size()); i++)
    {
        double *addr = parameter_blocks[residual_block_info->drop_set[i]]; // 获取要边缘化的参数块的首地址
        parameter_block_idx[reinterpret_cast<long>(addr)] = 0; // 要边缘化的参数块相对于首地址的ID，即第几个；
    }
}
