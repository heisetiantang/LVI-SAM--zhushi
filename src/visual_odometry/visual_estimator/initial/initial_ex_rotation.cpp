...
double InitialEXRotation::testTriangulation(const vector<cv::Point2f> &l,//上一帧特征点
                                          const vector<cv::Point2f> &r,//当前帧特征点
                                          cv::Mat_<double> R, //分解所得旋转矩阵
                                          cv::Mat_<double> t)//分解所得平移矩阵
{
    cv::Mat pointcloud;
    ...
    cv::triangulatePoints(P, P1, l, r, pointcloud);//三角化
    int front_count = 0;
    for (int i = 0; i < pointcloud.cols; i++)
    {
        double normal_factor = pointcloud.col(i).at<float>(3);//归一化
