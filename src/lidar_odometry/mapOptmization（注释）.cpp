...
public:
    ...
    {        
        ...
        if (cloudInfo.odomAvailable == true && cloudInfo.odomResetId == odomResetId)
        {
            ...
            else 
            {
                ...
                // 则将【当前】VINS IMU递推的里程计转换为Eigen::Affine3f的transBack最新VINS消息
                ...
                // Tvins_k-1 * Tvins_k = T_vins_k-1_k   ，上一帧到现在的位姿变换
        ...
        else// VINS IMU里程计不可用的情况
        {
            // ROS_WARN("VINS failure detected.");
            lastVinsTransAvailable = false;
            odomResetId = cloudInfo.odomResetId;//复位标
        }
    ...
    void extractNearby()
    {
        // 定义存储【当前帧】附近(指定半径范围内)的【关键帧位置信息】的点云指针(集合)，当前帧内50米内，并降采样
        ...
        */
        extractCloud(surroundingKeyPosesDS);//拼接
    }
    ...
    void extractCloud(pcl::PointCloud<PointType>::Ptr cloudToExtract)
    {
        ...
        // extract surrounding map，无顺序for循环可以并行
        ...
        for (int i = 0; i < (int)cloudToExtract->size(); ++i)
        {
            ...
            laserCloudCornerSurroundingVec[i]  = *transformPointCloud(cornerCloudKeyFrames[thisKeyInd],  &cloudKeyPoses6D->points[thisKeyInd]);//根据索引对于关键帧点云拼接
        ...
        for (int i = 0; i < (int)cloudToExtract->size(); ++i)
        {
            ...
            *laserCloudCornerFromMap += laserCloudCornerSurroundingVec[i];//用拼接的点云建造局部地图
        ...
        downSizeFilterSurf.setInputCloud(laserCloudSurfFromMap);//用这个构建约束
    ...
    bool saveFrame()
    {
        ...
        // 计算最新关键帧的位姿
        Eigen::Affine3f transStart = pclPointToAffine3f(cloudKeyPoses6D->back());//存储所有关键帧位姿的点云树结构，back表示最新的
        // 计算目前帧（不一定是关键帧）
    ...
    void addOdomFactor()
    {
        ...
        {
            // 设置先验噪声 yaw和位置方差设置较大
            // Diagonal类继承自Gaussian类，使用gtsam的接口，统一量纲
            noiseModel::Diagonal::shared_ptr priorNoise = noiseModel::Diagonal::Variances((Vector(6) << 1e-2, 1e-2, M_PI*M_PI, 1e8, 1e8, 1e8).finished()); // rad*rad, meter*meter
            // gtsam::NonlinearFactorGraph非线性因子图  （序号， 提供先验初值 ，先验位姿可信度）
    ...
    void addLoopFactor()
    {
        // vector<pair<int, int>> loopIndexQueue;存着两个回环帧的索引
        ...
        for (size_t i = 0; i < loopIndexQueue.size(); ++i)
        {
            ...
            // 获取【历史帧】到【新帧】的位姿约束
            // vector<gtsam::Pose3> loopPoseQueue;存贮gtsam位姿
    ...
    void correctPoses()
    {
        ...
        // 如果检测到回环
        if (aLoopIsClosed == true)//在关键帧判断是否有回环
        {
            // clear path
